    ...
    <div className={`bg-white dark:bg-slate-800 rounded-xl p-5 shadow-sm border transition-all ${hasAnyAlert ? 'border-red-200 dark:border-red-900/50 hover:shadow-md' : 'border-slate-100 dark:border-slate-700 hover:shadow-md'}`}>
      ...
      <div className="mb-4">
        ...
        <div className="space-y-1.5">
          {hasSaudeAlert ? (
            <div className="text-xs break-words text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2.5 py-1.5 rounded border border-red-100 dark:border-red-800/30">⚠️ Saúde: {child.saude?.alertas.join(', ')}</div>
          ) : (
          ...
          {hasEducacaoAlert ? (
            <div className="text-xs break-words text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2.5 py-1.5 rounded border border-red-100 dark:border-red-800/30">⚠️ Educação: {child.educacao?.alertas.join(', ')}</div>
          ) : (
          ...
          {hasSocialAlert ? (
            <div className="text-xs break-words text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2.5 py-1.5 rounded border border-red-100 dark:border-red-800/30">⚠️ Social: {child.assistencia_social?.alertas.join(', ')}</div>
          ) : (
